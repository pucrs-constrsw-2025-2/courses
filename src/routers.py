...
async def fetch_classes_from_api(
    class_ids: List[str], semester: Optional[int], year: Optional[int]
) -> List[ClassDTO]:
    if not class_ids:
        return []
...
async def create_course(course: CourseBase, token_data: dict = Depends(validate_token)):
    """Cria um novo curso."""
...
async def get_course(id: str, token_data: dict = Depends(validate_token)):
...
async def update_course(
    id: str, course: CourseBase, token_data: dict = Depends(validate_token)
):
    """Atualiza um curso completamente (substituição)."""
    await _get_course_or_404(id)  # Verifica se existe

    update_result = await course_collection.replace_one(
        {"_id": ObjectId(id)}, course.model_dump()
    )

    # Verifica se o replace_one realmente encontrou e atualizou o documento
    if update_result.matched_count == 0:
        raise HTTPException(
            status_code=404, detail=f"Course with ID {id} not found during update"
        )
...
async def partial_update_course(
    id: str, course: CourseUpdate, token_data: dict = Depends(validate_token)
):
    ...
    await course_collection.update_one({"_id": ObjectId(id)}, {"$set": update_data})
...
async def delete_course(id: str, token_data: dict = Depends(validate_token)):
    """Deleta um curso."""
    await _get_course_or_404(id)  # Verifica se existe
...
async def add_material_to_course(
    id: str, material: MaterialBase, token_data: dict = Depends(validate_token)
):
    ...
    await course_collection.update_one(
        {"_id": ObjectId(id)}, {"$push": {"materials": material_dict}}
    )
    return new_material
...
async def get_materials_from_course(
    id: str, name: Optional[str] = None, token_data: dict = Depends(validate_token)
):
...
async def get_material_from_course(
    id: str, material_id: str, token_data: dict = Depends(validate_token)
):
    ...
    raise HTTPException(
        status_code=404,
        detail=f"Material with ID {material_id} not found in course {id}",
    )
    ...
    update_result = await course_collection.update_one(
        {"_id": ObjectId(id), "materials.id": material_id},
        {
            "$set": {
                ...
                "materials.$.url": str(material.url),
            }
        },
    )
    if update_result.matched_count == 0:
...
@router.delete(
    "/courses/{id}/materials/{material_id}", status_code=status.HTTP_204_NO_CONTENT
)
async def delete_material_from_course(
    ...
    update_result = await course_collection.update_one(
        {"_id": ObjectId(id)}, {"$pull": {"materials": {"id": material_id}}}
    )
